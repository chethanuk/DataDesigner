...
<Note title="Historical post">
This post describes the model client stack as it shipped in v0.5.4. `ThrottledModelClient` and `ThrottleManager` have since been replaced by `ModelRequestExecutor` and request admission. `RunConfig.throttle` and `ThrottleConfig` are deprecated in favor of `RunConfig.request_admission` with `RequestAdmissionTuningConfig`, and the throttle log lines quoted below are no longer written. The ceiling stabilization described below has also changed: after a 429, the limit now climbs back to `max_parallel_requests`, and `rate_limit_ceiling` is kept only for diagnostics. See [Async Scheduling](/concepts/async-scheduling) for the current design.
</Note>
...
Data Designer now uses the async engine for generation, so 429s bypass transport retries and flow straight to `ThrottledModelClient` for AIMD feedback. See [Async All the Way Down](/dev-notes/async-all-the-way-down) for the full story on the async engine.
