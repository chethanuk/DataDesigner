...
This post describes the async engine as it launched in April 2026. The `ThrottleManager` layer and the submission/LLM-wait semaphore handoff described below have since been replaced by task admission and request admission. See [Async Scheduling](/concepts/async-scheduling) for the current design.
</Note>
...
The async engine is now Data Designer's execution engine. Your existing pipeline definitions, dependency graph, column configs, and model aliases stay the same.
