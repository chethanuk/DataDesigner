...
For Anthropic providers, `presence_penalty` is not sent. The [Anthropic Messages API](https://docs.anthropic.com/en/api/messages) supports `top_k` but has no `min_p` or `repetition_penalty`, so leave those two unset.
</Note>
