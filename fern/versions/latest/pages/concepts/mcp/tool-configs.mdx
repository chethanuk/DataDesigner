...
| `unknown_tool_fallback` | `bool` | No | Reply to calls for tools no provider offers with a tool message instead of failing the record (default: `False`) |
| `unknown_tool_message` | `str` | No | Text of that reply when `unknown_tool_fallback` is on (default: built-in message) |
