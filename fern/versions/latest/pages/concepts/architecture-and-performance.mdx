...
The default is `False`, the fixed horizon. Adaptive admission can keep fast endpoints busier on deep or uneven workflows, but as the target grows the run keeps more row data in memory and may delay the first checkpoint. With `write_scheduler_events=True`, `scheduler_events.jsonl` records the configured mode and starting target in `scheduler_job_started` and each target change as `row_group_admission_target_changed`. Benchmark before and after changing it.
