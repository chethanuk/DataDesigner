...
concurrent_requests = min(
    ...
    task_admission_slots,      # Task leases: max_in_flight_tasks overall, max_parallel_requests per model
    current_admission_limit,   # AIMD-managed request limit (≤ max_parallel_requests)
)
```

Work is admitted in two stages. **Task admission** decides when a ready cell may start: every task holds a lease against the global `max_in_flight_tasks` budget, and a task for a model column also holds one of `max_parallel_requests` task slots for its model. **Request admission** then decides when each API call may go out. A task keeps its lease while it waits for request admission. See [Async Scheduling](/concepts/async-scheduling) for how the stages fit together.
...
- **After consecutive successes**: the limit increases by 1 (by default) until it is back at `max_parallel_requests`.
...
Data Designer does not store the complete effective `RunConfig`. `metadata.json` records selected resume identity values, including `buffer_size`, `preserve_dropped_columns`, the target record count, and dataset-config identity fields; `builder_config.json` stores the dataset builder configuration. Resume scans completed `batch_*.parquet` row groups and reads parquet metadata for the row count actually persisted. That keeps resume crash-safe even if a run was interrupted between writing a row-group parquet and updating metadata, because the filesystem reflects the durable state even when metadata lags behind.
...
Limit changes are not written to the log. With `display_tui=True`, the throughput chart marks each rate-limit signal. For a record you can read after the run, set `write_scheduler_events=True` and check `diagnostics.request_pressure` in the `scheduler_health_snapshot` events of `scheduler_events.jsonl`, which shows each model's current limit, maximum, waiters, and cooldown. See [Async Scheduling](/concepts/async-scheduling#telemetry-which-layer-is-limiting).
</Tip>
...
The async engine is the execution path. It dispatches work at the cell level rather than the column level, so independent columns overlap in time and the AIMD limit for each provider, model, and request type adjusts on its own. See [Async Scheduling](/concepts/async-scheduling) for the full architecture.
...
|---------|---------|----------|
| **Low throughput** | Low GPU utilization | Increase `max_parallel_requests` and/or `buffer_size`. If request admission has reduced concurrency after earlier 429s (check `request_pressure` in `scheduler_events.jsonl`), the server may need more capacity or you can wait for AIMD recovery. |
| **Frequent 429 → recovery cycles** | Repeated rate-limit markers in the TUI, or `current_limit` repeatedly below its maximum in `scheduler_events.jsonl` | The `max_parallel_requests` ceiling is above the server's sustained capacity. This is handled automatically, but you can lower the ceiling to reduce the sawtooth or tune `multiplicative_decrease_factor` / `successes_until_increase`. |
...
4. **Check request admission**: Turn on `write_scheduler_events` and read `request_pressure` in `scheduler_events.jsonl` to see whether rate limits are the bottleneck
...
- [Async Scheduling](/concepts/async-scheduling): Task admission, request admission, and scheduler telemetry
