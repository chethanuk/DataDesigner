...
**ALL doc prose edits happen under `fern/`.** Notebook source, generated Colab notebooks, docs scripts, and downloadable recipe scripts also live under `fern/` (see `fern/README.md`). `fern/notebook_source/*.py` is canonical for notebook code. Concept pages, recipes, plugins, and Dev Notes prose live under `fern/versions/latest/pages/`.
...
```
fern/notebook_source/*.py            (jupytext format — canonical source, edit here)
        │ make convert-execute-notebooks      # jupytext --execute (needs NVIDIA_API_KEY)
        ▼
fern/notebooks/*.ipynb               (executed; outputs captured)
        │
        │ make generate-fern-notebooks         # per-file prefers executed fern/notebooks/
...
`fern/colab_notebooks/*.ipynb` is a separate committed output for "Open in Colab" links. It is generated by `make generate-colab-notebooks`, but it is not a Fern docs build input.
...
| Command | When |
|---------|------|
| `make generate-fern-notebooks` | Notebook prose changed, no need to re-execute. Per file, prefers `fern/notebooks/` (executed) and falls back to converting `fern/notebook_source/*.py` directly. |
...
- Adding a notebook tutorial's *code*: edit `fern/notebook_source/<name>.py`, not the converted `.ipynb` or the wrapper MDX.
